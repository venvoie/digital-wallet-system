	...
	public partial class Dashboard : Page
	{
		...
		private void LoadDashboard()
		{
			...
			using (SqlConnection conn = new SqlConnection(connStr))
			{
				...
				string sqlUser = @"
                    SELECT  u.FullName, u.AccountNumber, u.DateRegistered,
                            w.CurrentBalance, w.TotalSentAmount
                    FROM    Users u JOIN Wallets w ON w.UserID = u.UserID
				...
				using (SqlCommand cmd = new SqlCommand(sqlUser, conn))
				{
					...
					if (dr.Read())
					{
						// populate the hero balance card labels
						...
						master?.SetUserInfo(dr["FullName"].ToString(), dr["AccountNumber"].ToString());
					}
				...
				using (SqlCommand cmd = new SqlCommand(sqlStats, conn))
				{
					...
					while (dr.Read())
					{
						...
						switch (type)
						{
							case "R": lblTotalReceived.Text = total.ToString("N2"); lblReceivedCount.Text = count.ToString(); break;
							case "D": lblTotalDeposited.Text = total.ToString("N2"); lblDepositCount.Text = count.ToString(); break;
							case "W": lblTotalWithdrawn.Text = total.ToString("N2"); lblWithdrawCount.Text = count.ToString(); break;
							case "S": lblSentCount.Text = count.ToString(); break;
						}
					}
				...
				// fetch the 5 most recent received transactions for the notifications panel
				string sqlNotifs = @"
                    SELECT  TOP 5 t.Amount, t.TransactionDate, u.FullName AS SenderName
                    FROM    Transactions t JOIN Users u ON u.UserID = t.ReceivedFromUserID
                    WHERE   t.UserID = @UserID AND t.TransactionType = 'R'
				...
				using (SqlCommand cmd = new SqlCommand(sqlNotifs, conn))
				{
					...
					// show empty state if no received transactions exist
					if (dt.Rows.Count == 0) pnlNoNotifs.Visible = true;
					else rptNotifications.DataSource = dt;

					rptNotifications.DataBind();
				}

				// fetch the 5 most recent transactions of any type for the activity panel
				string sqlTxn = @"
                    SELECT  TOP 5 TransactionType, Amount, TransactionDate
                    FROM    Transactions
                    WHERE   UserID = @UserID
				...
				using (SqlCommand cmd = new SqlCommand(sqlTxn, conn))
				{
					...
					if (dt.Rows.Count == 0) pnlNoTxn.Visible = true;
					else rptTransactions.DataSource = dt;
		...
		protected bool IsCredit(string type) => type == "D" || type == "R";
	}
}