...
namespace DigitalWalletSystem.Pages.Reports
{
	public partial class SentReceived : Page
	{
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			// pre-fill date fields and load transactions using the default range and type
		...
		// queries and binds sent/received transactions for the given date range and type filter
		private void LoadTransactions(int userID, string connStr, DateTime fromDate, DateTime toDate, string type)
		{
			...
			string sql = @"SELECT t.TransactionType, t.TransactionDate, t.Amount,
			...
			if (type == "S") sql += " AND t.TransactionType = 'S'";
			else if (type == "R") sql += " AND t.TransactionType = 'R'";
			...
			using (var cmd = new SqlCommand(sql, conn))
			{
				...
				string typeLabel = type == "S" ? "Sent" : type == "R" ? "Received" : "All";

				// display active date range and type in the section header
				lblDateRange.Text = $"{fromDate:MMM dd, yyyy} — {toDate:MMM dd, yyyy} · {typeLabel}";
			}
		}

		// shows error panel with message and hides results
